...
import { listLessons, loadLessonHtml } from '@/lib/content'

export default async function LessonPage({ params }: { params: { course: string, lesson: string } }) {
  const { meta, html } = await loadLessonHtml(params.course, params.lesson)
    ...
    <div className="grid grid-cols-12 gap-6">
      <aside className="col-span-12 md:col-span-3 border-r pr-4">
        ...
        <ol className="mt-2 space-y-1 text-sm">
          {lessons.map(l => (
            <li key={l.id}>
              <Link className={l.slug===meta.slug ? 'underline' : ''} href={`/${l.course}/${l.slug}`}>{l.order}. {l.title}</Link>
            </li>
          ))}
        </ol>
      </aside>
      <article className="col-span-12 md:col-span-9 space-y-4">
        ...
        <div className="p-3 rounded bg-amber-50 dark:bg-amber-900/30 text-sm">
          Runner & auto‑grading arrive in Phase 1. For now, read the concept.
        </div>
